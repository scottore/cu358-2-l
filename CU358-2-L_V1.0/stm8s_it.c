...
/**
 ...
 INTERRUPT_HANDLER(UART1_TX_IRQHandler, 17)
 {
    /* In order to detect unexpected events during development,
       it is recommended to set a breakpoint on the following instruction.
    */
    if((UART1->SR & (uint8_t)UART1_SR_TC) !=  (uint8_t)0x00)
   {
     uart1TrvCompleteCallBack();
...
/**
 ...
 INTERRUPT_HANDLER(UART1_RX_IRQHandler, 18)
 {
  ...
  if ((UART1->SR & (uint8_t)UART1_SR_OR) != (uint8_t)0x00)
  {
    UART1->SR = (uint8_t)~(UART1_SR_OR);//清除ORE
  }
   uint8_t Rdata = (uint8_t)UART1->DR;
   uart1ReceiveCallback(Rdata);
...
/**
  ...
  */
    sys_1ms_timeout();
   // while((TIM4->SR1 & 0x81) == 0x00);      // 等待更新标志
